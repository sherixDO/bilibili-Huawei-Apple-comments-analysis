...
# =======================================================================
### 修改点 1: 使用为苹果定制的、高强度的Prompt模板 ###
# =======================================================================
NEW_PROMPT_TEMPLATE= """你是一个专门用于情感分析的API。你的唯一工作是接收一段关于苹果(Apple)产品的用户评论，然后返回一个严格的JSON对象。绝对不允许输出任何JSON以外的文字。

JSON对象必须包含以下两个键:
1. `sentiment`: 字符串类型，值必须是 "正面"、"负面" 或 "中性" 之一。
2. `reason`: 字符串类型，简明扼要地解释你做出判断的核心理由，需要识别出讽刺和隐藏含义。
...
---
评论: 『太环保了，我哭死，为了地球我选择再单独买个充电器。』
```json
{{
  "reason": "这是一个关于苹果不再附赠充电器的网络梗。评论者使用'环保'、'哭死'等词语进行反讽，真实意图是抱怨苹果以环保为名，迫使用户额外消费，属强烈负面。",
  "sentiment": "负面"
}}```
---
...
# =======================================================================
### 修改点 2: 使用最健壮的get_analysis_batch函数，并确保它调用苹果模板 ###
# =======================================================================
...
def parse_arguments() -> argparse.Namespace:
    ...
    # 推荐更换为Chat/Instruct模型以获得更稳定的JSON输出
    parser.add_argument("--model-path", type=str, default="/root/autodl-tmp/DeepSeek-R1-Qwen2.5-Math-14B",
                        help="大语言模型路径。推荐使用Chat或Instruct版本。")

    # 更改默认文件名和品牌名
    parser.add_argument("--input-csv", type=str, default="apple.csv", help="待分析的输入CSV文件。")
    parser.add_argument("--output-csv", type=str, default="apple_analyzed_results.csv",
                        help="保存分析结果的输出CSV文件。")
    parser.add_argument("--brand-name", type=str, default="苹果", help="分析报告中显示的品牌名。")
    ...
    parser.add_argument("--batch-size", type=int, default=32, help="进行LLM推理的批大小。可根据显存调整。")
